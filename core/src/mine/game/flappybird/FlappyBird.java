...
public class FlappyBird extends ApplicationAdapter {
	...
	int numberOfTube = 3;
	float gap = 380;
	int randomGapY[] = new int[numberOfTube];
	float[] tubeX = new float[numberOfTube];
	float tubeVelocity = 4;
	float distanceBetweenTubes = 550;
	int scores = 0;
	int scoringTube = 0;
...
//		distanceBetweenTubes = Gdx.graphics.getWidth() / 2;
		for(int i = 0; i<numberOfTube; i++) {
			tubeX[i] = Gdx.graphics.getWidth() + topTube.getWidth() + i * distanceBetweenTubes;
			randomGapY[i] = getRandomNumber();
	...
	public void render () {
		...
		if(gameState == 1){
			...
			for(int i = 0; i<numberOfTube; i++){
				...
				batch.draw(topTube, tubeX[i], randomGapY[i] + gap);
				batch.draw(bottomTube, tubeX[i], randomGapY[i] - bottomTube.getHeight());
				...
				bottomTubeRectangle[i] = new Rectangle(tubeX[i],
						randomGapY[i] - bottomTube.getHeight(),
						bottomTube.getWidth(),
						topTube.getHeight());
			}
		...
		bitmapFont.draw(batch, Integer.toString(scores), 100,150);

		batch.end();

		circle.set(Gdx.graphics.getWidth() / 2,
				birdY + birds[flapState].getWidth() / 2 - 15,
...
//						topTube.getHeight());
	...
	public int getRandomNumber() {
		int min = 100;
		int max = (int)(Gdx.graphics.getHeight()-gap-100);
